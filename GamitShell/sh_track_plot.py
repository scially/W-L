...
def plots(resfile, offset=0, col=6, **kwargs):
    ...
    plt.plot(range(0,North.shape[0],1), 100 * (North-North[0]) + offset, label=os.path.basename(resfile)[-7:-3].upper())
    plt.errorbar(range(0,North.shape[0],1),100 * (North-North[0]) + offset,yerr=dNorth*100,ecolor='pink',errorevery=10,alpha=0.1)
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        Usage:
            ./sh_track_plot.py -out <out> -start <start> -offset <offset>
    ...
    for file in os.listdir(os.getcwd()):
        if file[-2:] != 'LC':
            continue
        plots(os.path.join(os.getcwd(), file), i * 100,8, loc='upper right', label='East(mm)'
              , start_time=start, offset_time=offset)
        i = -1 * (abs(i) + 1)
    plt.savefig(out)
    plt.show()
